...
public final class BatterySim {
  ...
  private static List<DoubleSupplier> m_currentSources = new ArrayList<DoubleSupplier>();
   ...
   */
  public static double calculateDefaultBatteryLoadedVoltage() {
    return calculateLoadedBatteryVoltage(12, 0.02);
  }
  /**
   * Calculate the loaded battery voltage. Use this with {@link RoboRioSim#setVInVoltage(double)} to
   * set the simulated battery voltage, which can then be retrieved with the {@link
   ...
   * @return The battery's voltage under load.
   */
  public static double calculateLoadedBatteryVoltage(double nominalVoltage, double resistanceOhms) {
    double current = 0.0;
    for (DoubleSupplier source : m_currentSources) {
        current += source.getAsDouble();
    }
    return nominalVoltage - (current * resistanceOhms);
  }

  public static void addCurrentSource(DoubleSupplier currentSource) {
    m_currentSources.add(currentSource);
  }
}
